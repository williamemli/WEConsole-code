...
async function launchDsRom(romName: string) {
  // Pausa musiken när ett spel startas
  playerStore.pause();
  // Skicka bara filnamnet till backend, inte sökvägen
  try {
    await invoke('launch_ds_game', { romName });
  } catch (e: any) {
    console.error('Failed to launch game:', e, 'ROM name:', romName);
